...
def ellipsoid_cut(r = 1.0, origin=(0,0,0), h=0.1, d=(1,0,0), angle=0, new_axis=(1,0,0)):

    stub = '''
    Point(1) = {c1 + r*Cos(7*Pi/10), c2 + r*0.5*Sin(7*Pi/10), c3, refine};
    Point(2) = {c1 + r*1, c2, c3, refine};
    Point(3) = {c1 + r*Cos(7*Pi/10), c2 + r*(-0.5*Sin(7*Pi/10)), c3, refine};

    phi = Acos(Cos(7*Pi/10)/1.3);
    Point(4) = {c1 + r*1.3*Cos(phi), c2 + r*0.6*Sin(phi), c3, refine};
    Point(5) = {c1 + r*1.3*Cos(phi), c2 + r*(-0.6*Sin(phi)), c3, refine};
    Point(6) = {c1 + r*1.3, c2, c3, refine};
    Point(7) = {c1, c2, c3, refine};

    Rotate {{d1,d2,d3}, {c1,c2,c3}, theta}{
        Point{:}; 
    }
    ...
    extr1[] = Extrude {{d_new_1,d_new_2,d_new_3}, {c1,c2,c3}, Pi/2} {
        Line{2,3,1};
    };

    extr2[] = Extrude {{d_new_1,d_new_2,d_new_3}, {c1,c2,c3}, Pi/2} {
        Line{4,7,11};
    };

    extr3[] = Extrude {{d_new_1,d_new_2,d_new_3}, {c1,c2,c3}, Pi/2} {
        Line{14,17,21};
    };

    extr4[] = Extrude {{d_new_1,d_new_2,d_new_3}, {c1,c2,c3}, Pi/2} {
        Line{24,27,31};
    };
    '''
