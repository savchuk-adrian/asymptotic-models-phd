...
def get_real_imag_of_omega_k(omega, k):
    r = _np.abs(omega) 
    phi = _np.angle(omega) 
    
    omega_k = r**k * _np.exp(1j * k * phi)

    real_part = _np.real(omega_k)
    imag_part = _np.imag(omega_k)
...
def custom(
    ...
    from .modified_helmholtz import custom as _modified_custom 

    # if _np.real(wavenumber) == 0:
    #     return _modified_custom(
    #         domain,
    #         range_,
    #         dual_to_range,
    #         _np.imag(wavenumber),
    ...
    # real_omega, imag_omega = get_real_imag_of_omega_k(1j*wavenumber, order)
    ...
    return _common.create_operator(
        ...
        parameters,
        assembler,
        [epsilon, center[0], center[1], center[2], order],
        "custom",
        "default_scalar",
        device_interface,
        precision,
...
def single_layer(
    ...
    return _common.create_operator(
        ...
        parameters,
        assembler,
        [_np.real(wavenumber), _np.imag(wavenumber), epsilon, c_k[0], c_k[1], c_k[2], c_l[0], c_l[1], c_l[2]],
...
def single_layer_coupled(
    ...
    """Assemble the Helmholtz single-layer boundary operator."""
    from .modified_helmholtz import single_layer_coupled as _modified_single_layer
    ...
    return _common.create_operator(
        ...
        parameters,
        assembler,
        [_np.real(wavenumber), _np.imag(wavenumber), epsilon, c[0], c[1], c[2]],
        "helmholtz_single_layer_coupled",
        "default_scalar",
        device_interface,
        precision,
        True,
