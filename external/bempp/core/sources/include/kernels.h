...
inline void diff_vec_scaled(const REALTYPE3 vec1, const REALTYPEVEC vec2[3], 
                            REALTYPE epsilon, const REALTYPE3 c_k, const REALTYPE3 c_l, 
                            REALTYPEVEC result[3]) 
{
    result[0] = epsilon * (vec2[0] - vec1.x) + epsilon * (c_l.x - c_k.x) + (c_k.x - c_l.x);
    result[1] = epsilon * (vec2[1] - vec1.y) + epsilon * (c_l.y - c_k.y) + (c_k.y - c_l.y);
    result[2] = epsilon * (vec2[2] - vec1.z) + epsilon * (c_l.z - c_k.z) + (c_k.z - c_l.z);
}

inline void diff_vec4_scaled(const REALTYPE3 vec1, const REALTYPE4 vec2[3], 
                            REALTYPE epsilon, const REALTYPE3 c_k, const REALTYPE3 c_l, 
                            REALTYPE4 result[3]) 
{
    result[0] = epsilon * (vec2[0] - vec1.x) + epsilon * (c_l.x - c_k.x) + (c_k.x - c_l.x);
...
inline void diff_vec8_scaled(const REALTYPE3 vec1, const REALTYPE8 vec2[3], 
                            REALTYPE epsilon, const REALTYPE3 c_k, const REALTYPE3 c_l, 
                            REALTYPE8 result[3]) 
{
    result[0] = epsilon * (vec2[0] - vec1.x) + epsilon * (c_l.x - c_k.x) + (c_k.x - c_l.x);
...
inline void diff_vec16_scaled(const REALTYPE3 vec1, const REALTYPE16 vec2[3], 
                            REALTYPE epsilon, const REALTYPE3 c_k, const REALTYPE3 c_l, 
                            REALTYPE16 result[3]) 
{
    result[0] = epsilon * (vec2[0] - vec1.x) + epsilon * (c_l.x - c_k.x) + (c_k.x - c_l.x);
                                                      ...
                                                      REALTYPE4* result)
{
    ...
    // Використовуємо epsilon і координати центрів c_l та c_k
    REALTYPE epsilon = kernel_parameters[2];   // epsilon
    REALTYPE3 c_k = { kernel_parameters[3], kernel_parameters[4], kernel_parameters[5] }; // центри c_l
    REALTYPE3 c_l = { kernel_parameters[6], kernel_parameters[7], kernel_parameters[8] }; // центри c_k

    // Використовуємо масштабовану різницю
    diff_vec4_scaled(testGlobalPoint, trialGlobalPoint, epsilon, c_k, c_l, diff);
                                                      ...
                                                      REALTYPE8* result)
{
    ...
    // Використовуємо масштабовану різницю
    diff_vec8_scaled(testGlobalPoint, trialGlobalPoint, epsilon, c_k, c_l, diff);
                                                       ...
                                                       REALTYPE16* result)
{
    ...
    // Використовуємо масштабовану різницю
    diff_vec16_scaled(testGlobalPoint, trialGlobalPoint, epsilon, c_k, c_l, diff);
