...
def select_numba_kernels(operator_descriptor, mode="regular"):
    ...
    kernel_functions_regular = {
        ...
        "helmholtz_single_layer_scaled": helmholtz_single_layer_regular_scaled, #!
        ...
        "modified_helmholtz_single_layer_scaled": modified_helmholtz_single_layer_regular_scaled, #!
        "helmholtz_single_layer_coupled": helmholtz_single_layer_regular_scaled, #! 
        "modified_helmholtz_single_layer_coupled": modified_helmholtz_single_layer_regular_scaled, #!
    ...
    kernel_functions_singular = {
        ...
        # "helmholtz_single_layer_coupled": helmholtz_single_layer_coupled_singular,
        # "modified_helmholtz_single_layer_coupled": modified_helmholtz_single_layer_coupled_singular, 
...
def custom_regular(
    ...
    """Evaluate custom operator"""
    epsilon = kernel_parameters[0]
    c = [kernel_parameters[1], kernel_parameters[2], kernel_parameters[3]]
    k = kernel_parameters[4]
    ...
    for i in range(3):
        for j in range(npoints):
            dist[j] += (epsilon*(trial_points[i, j] - test_point[i]) - c[i]) ** 2
    for j in range(npoints):
        dist[j] = _np.sqrt(dist[j])

    for j in range(npoints):
        output_real[j] = dist[j]**(k-1) 
...
def custom_singular(
    ...
    for i in range(3):
        for j in range(npoints):
            rad[j] += (epsilon*(trial_points[i, j] - test_points[i, j]) - c[i]) ** 2 
    for j in range(npoints):
        rad[j] = _np.sqrt(rad[j])

    for j in range(npoints):
        output_real[j] = rad[j]**(k-1) 
...
def helmholtz_single_layer_regular(
    ...
    epsilon = kernel_parameters[2]
    c_k = [kernel_parameters[3],kernel_parameters[4],kernel_parameters[5]]
    c_l = [kernel_parameters[6],kernel_parameters[7],kernel_parameters[8]]
    npoints = trial_points.shape[1]
    dtype = trial_points.dtype
    dist = _np.zeros(npoints, dtype=dtype)
    output_real = _np.zeros(npoints, dtype=dtype)
    ...
    for i in range(3):
        for j in range(npoints):
            dist[j] += (epsilon*(trial_points[i, j] - test_point[i]) + epsilon*(c_l[i] - c_k[i]) + c_k[i] - c_l[i]) ** 2
    for j in range(npoints):
        dist[j] = _np.sqrt(dist[j])
...
def helmholtz_single_layer_singular(
    ...
    for i in range(3):
        for j in range(npoints):
            rad[j] += (epsilon*(trial_points[i, j] - test_points[i, j]) + epsilon*(c_l[i] - c_k[i]) + c_k[i] - c_l[i]) ** 2 
    for j in range(npoints):
        rad[j] = _np.sqrt(rad[j])
...
def helmholtz_single_layer_regular_scaled(
    test_point, trial_points, test_normal, trial_normals, kernel_parameters
):
    """Evaluate Helmholtz single layer for regular kernels."""
    wavenumber_real = kernel_parameters[0]
    ...
    output_imag = _np.zeros(npoints, dtype=dtype)
    m_inv_4pi = dtype.type(M_INV_4PI)
    c = [kernel_parameters[3],kernel_parameters[4],kernel_parameters[5]]
    for i in range(3):
        for j in range(npoints):
            dist[j] += (epsilon*(trial_points[i, j]-c[i]) + c[i] - test_point[i]) ** 2 
    for j in range(npoints):
        dist[j] = _np.sqrt(dist[j])
...
def modified_helmholtz_single_layer_regular(
    ...
    epsilon = kernel_parameters[1]
    c_k = [kernel_parameters[2],kernel_parameters[3],kernel_parameters[4]]
    c_l = [kernel_parameters[5],kernel_parameters[6],kernel_parameters[7]]
    for i in range(3):
        for j in range(npoints):
            dist[j] += (epsilon*(trial_points[i, j] - test_points[i]) + epsilon*(c_l[i] - c_k[i]) + c_k[i] - c_l[i]) ** 2 
    for j in range(npoints):
        dist[j] = _np.sqrt(dist[j])
    for j in range(npoints):
...
def modified_helmholtz_single_layer_singular(
    ...
    for i in range(3):
        for j in range(npoints):
            dist[j] += (epsilon*(trial_points[i, j] - test_points[i, j]) + epsilon*(c_l[i] - c_k[i]) + c_k[i] - c_l[i])**2 
    for j in range(npoints):
        dist[j] = _np.sqrt(dist[j])
    for j in range(npoints):
        ewr[j] = _np.exp(-kernel_parameters[0] * dist[j])
...
def modified_helmholtz_single_layer_regular_scaled(
    test_points, trial_points, test_normal, trial_normal, kernel_parameters
):
    """Evaluate Modified Helmholtz single layer for regular kernels."""
    npoints = trial_points.shape[1]
    ...
    # my parameters 
    epsilon = kernel_parameters[1]
    c = [kernel_parameters[2],kernel_parameters[3],kernel_parameters[4]]
    for i in range(3):
        for j in range(npoints):
            dist[j] += (epsilon*(trial_points[i, j]-c[i]) + c[i] - test_points[i]) ** 2 
    for j in range(npoints):
        dist[j] = _np.sqrt(dist[j])
    for j in range(npoints):
        ewr[j] = _np.exp(-kernel_parameters[0] * dist[j])
